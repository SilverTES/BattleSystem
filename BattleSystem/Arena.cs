...
namespace BattleSystem
{
    public class Arena : Node
    {
        ...
        #region Attributes
        public State<States> State { get; private set; } = new State<States>(States.PhasePlayer);
            ...
            switch (State.CurState)
            {
                ...
                case States.PhaseEnemy:
                    foreach (var node in GroupOf(UID.Get<Card>()))
                    {
                        ...
                        card.State.Change(Card.States.IsWait);
                    }
                    break;
        ...
        protected void RunState(GameTime gameTime)
        {
            switch (State.CurState)
            {
        ...
        public override Node Draw(SpriteBatch batch, GameTime gameTime, int indexLayer)
        {
            ...
            switch ((Layers)indexLayer)
            {
                ...
                case Layers.Debug:
                    ...
                    GFX.LeftTopString(batch, Game1._fontMain, $"{State.CurState} {_isMouseOverGrid}", AbsRectF.BottomLeft + new Vector2(10, 10), Color.AntiqueWhite);
                    break;

...
namespace BattleSystem.Gui
{
    ...
    public class BaseGui : Node
    {
        ...
        public State<States> State { get; private set; } = new State<States>(States.Default);
        ...
        protected Color[] _textColors = new Color[(int)States.Count];
        protected Color[] _bgColors = new Color[(int)States.Count];

        protected BaseImage[] _bgImages = new BaseImage[(int)States.Count];
        protected BaseImage[] _fgImages = new BaseImage[(int)States.Count];
        ...
        public virtual Node SetStyle(JObject style)
        {
            ...
            for (int i = 0; i < (int)States.Count; i++)
            {
                _bgImages[i] = BaseImage.Load(style, "bgImage." + (States)i);
                _fgImages[i] = BaseImage.Load(style, "fgImage." + (States)i);

                _textColors[i] = Style.ColorValue.GetColor(Token.Get<string>(style, "textColor." + (States)i));
                _bgColors[i] = Style.ColorValue.GetColor(Token.Get<string>(style, "bgColor." + (States)i));
            }
        ...
        public override Node Update(GameTime gameTime)
        {
            ...
            if (!_navi._isMouseOver)
            {
                ...
                State.Change((int)States.Default);
            }
        ...
        protected virtual Node RenderImage(SpriteBatch batch, BaseImage[] images)
        {
            ...
            if (_navi._isMouseOver)
            {
                image = images[(int)States.Over];

                if (image == null)
                    GFX.FillRectangle(batch, AbsRect, _bgColors[(int)States.Over]);
            }
            else
            {
                image = images[(int)States.Default];

                if (image == null)
                    GFX.FillRectangle(batch, AbsRect, _bgColors[(int)States.Default]);
            }

            if (_navi._isPress)
            {
                image = images[(int)States.IsPress];

                if (image == null)
                    GFX.FillRectangle(batch, AbsRect, _bgColors[(int)States.IsPress]);
            }
        ...
        public override Node Draw(SpriteBatch batch, GameTime gameTime, int indexLayer)
        {
            if (indexLayer == (int)Layers.Gui)
            {
                ...
                if (_font != null)
                {
                    GFX.CenterStringXY(batch, _font, _label, AbsX + _animateVec2.Value().X, AbsY + 2 + _animateVec2.Value().Y, _textColors[(int)States.Default]);
                }
            ...
            if (indexLayer == (int)Layers.FrontFX)
            {
                ...
                if (_navi._isMouseOver)
                {
                    fgImage = _fgImages[(int)States.Over];
                }
                else
                {
                }
                ...
                if (_navi._isPress)
                {
                    fgImage = _fgImages[(int)States.IsPress];
                }
    ...
    public class CheckBox : BaseGui
    {
        ...
        protected BaseImage[] _checkedImages = new BaseImage[(int)States.Count];
        ...
        public override Node SetStyle(JObject style)
        {
            for (int i = 0; i < (int)States.Count; i++)
            {
                _checkedImages[i] = BaseImage.Load(style, "checkedImage." + (States)i);
            }

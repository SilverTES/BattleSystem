...
namespace BattleSystem
{
    public class Card : Node
    {
        ...
        public Card(Arena arena, bool isDropped = true, float tempoBeforeSpawn = 0f) 
        {
            ...
            State.Set(States.IsNull);

            if (tempoBeforeSpawn == 0)
                State.Set(States.IsSpawn);
        }
        ...
        {
            if (!_arena.IsCardInMap(this, mapStep) || State.CurState == States.IsMove)
                return false;
        ...
        {
            if (State.CurState != States.IsPlay)
                return;
        ...
        public override Node Update(GameTime gameTime)
        {
            ...
            if (_specs.Energy <= 0 && State.CurState != States.IsDead)
            {
                State.Change(States.IsDead);
            ...
            _isVisible = State.CurState != States.IsNull;
        ...
        public override Node Draw(SpriteBatch batch, GameTime gameTime, int indexLayer)
        {
            switch ((Layers)indexLayer)
            {
                ...
                case Layers.Main:
                    ...
                    if (State.CurState == States.IsDamaged)
                        color = Color.IndianRed * 1f;

                    if (State.CurState == States.IsDead)
                        color = Color.Red;
                ...
                case Layers.BackFX:
                    ...
                    if (State.CurState == States.IsMove)
                    {
